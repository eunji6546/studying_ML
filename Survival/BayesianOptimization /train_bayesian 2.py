...
# BO for mse of regression VERSION!
# data : just concat -> ae -> pca 
...
def run(gpunum, cancer_type, feature_type, attempt):
    ...
    def scoreofModel(cancer_type, feature_type, attempt):
        ...
        def inner_SoM( pca, dropout, ae_dim1, ae_dim2):
            ...
            def ModelV2(model_input):
                ...
                sgd = SGD(lr=learning_ratio, decay=1e-5, momentum=0.9, nesterov=True)        
            ...
            print("x_trn %s, x_dex %s, x_tst %s" %(str(x_trn.shape), str(x_dev.shape), str(x_tst.shape)))
            ...
            if variables.model_type == "V1":
                ...
                if variables.train_with_censored == "EXCLUDE":
                    ...
                    s_trn = s_trn[c_trn == 0]
                    s_dev = s_dev[c_dev == 0]
                    print("reduced to x_trn %s, x_dev %s" %(str(x_trn.shape), str(x_dev.shape)))
            ...
            checkpoint = MyCallback(results, model_filepath, data, real_save=True, verbose=0,
                                save_best_only=True,
    ...
    result_cindex = scoreofModel(cancer_type,feature_type,0)(0.9999, 0.0,1400, 700)
    ...
    bo_dict = { "pca" : (0.99, 0.9999), "dropout" : (0, 0.8), 
       # "hidden_dims" : (10, 1000), 
        "ae_dim1" : (1400, 1500), 
        "ae_dim2" : (500,700)
    }
